...
public class RtfConverter {
    ...
    private String buildDocument(String content) {
        ...
        for(int i = 0; i < colors.size(); i++) {
            document.append("\\red");
            document.append(colors.get(i).getRed());
            document.append("\\green");
            document.append(colors.get(i).getGreen());
            document.append("\\blue");
            document.append(colors.get(i).getBlue());
            document.append(";");
        }
    ...
    private void buildToken(String text, AttributeSet as, StringBuilder sb) {
        ...
        boolean underline = as.getAttribute(StyleConstants.Underline) != null;
        boolean strikethrough = as.getAttribute(StyleConstants.StrikeThrough) != null;
     ...
     */
    private void charsToRTF(String chars, StringBuilder sb) {
        for (int i = 0; i < chars.length(); i++) {
            if (chars.charAt(i) > 0xFF) {
                sb.append("{\\u");
                sb.append((short)chars.charAt(i));
                sb.append("}");
